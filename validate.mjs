...
const base=path.resolve('dist');const errors=[];let pages=0,links=0,images=0;
function walk(dir){return readdirSync(dir).flatMap(n=>{const p=path.join(dir,n);return statSync(p).isDirectory()?walk(p):[p];});}
for(const file of walk(base).filter(f=>f.endsWith('.html'))){pages++;const html=readFileSync(file,'utf8');
